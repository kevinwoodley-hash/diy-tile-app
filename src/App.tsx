...
function VoiceInput({ onResult, className = "" }) {
  ...
  React.useEffect(() => {
    ...
    r.onresult = (e) => {
      ...
      // Extract first number spoken (handles "three point five", "3.5", "four metres" etc.)
      const wordsToNum = { zero:0, one:1, two:2, three:3, four:4, five:5, six:6, seven:7, eight:8, nine:9, ten:10 };
      let text = raw.toLowerCase().replace(/metres?|meters?|m/g, "").trim();
      // replace word numbers
      text = text.replace(/(zero|one|two|three|four|five|six|seven|eight|nine|ten)/g, m => wordsToNum[m]);
      text = text.replace(/point/g, ".").replace(/and/g, "").replace(/\s+/g, "");
      const num = parseFloat(text);
      if (!isNaN(num)) onResult(String(num));
      setListening(false);
    };
  ...
  return (
    <button
      type="button"
      onClick={toggle}
      title={listening ? "Tap to stop" : "Tap to speak measurement"}
      style={{
        background: listening ? "#EF4444" : "#F3F4F6",
        border: `2px solid ${listening ? "#EF4444" : "#D1D5DB"}`,
        borderRadius: "8px",
        padding: "0 10px",
        height: "40px",
        cursor: "pointer",
...
export default function TilerQuoteApp() {
  ...
  rooms.forEach((room) => {
    ...
    if (room.useFourWallCalc) {
      ...
      const height = Number(room.roomHeight || 0);
      const perimeter = 2 * (length + width);
      totalWallArea += perimeter * height - Number(room.fourWallDeduct || 0);

    } else {
      room.walls.forEach((wall) => {
        totalWallArea +=
          Number(wall.length || 0) * Number(wall.height || 0) -
          Number(wall.deduct || 0);
      });
    }
  });
  ...
  const labourCost = rates.labourMode === "day"
    ...
    : totalFloorArea * rates.floorRate + totalWallArea * rates.wallRate;
  ...
  // UFH areas need flexible S1 adhesive at reduced coverage (~3m² per bag due to mat profile)
  const ufhAdhesiveCoverage = 3;
  const ufhAdhesiveBags = ufhM2 > 0 ? Math.ceil(ufhM2 / ufhAdhesiveCoverage) : 0;
  const standardTileArea = totalTileArea - ufhM2;
  const standardAdhesiveBags = Math.ceil(standardTileArea / adhesiveCoveragePerBag);
  ...
  /* ---- GROUT CALCULATION ----
     Formula: kg/m2 = ((L+W) / (LxW)) x jointWidth x tileThickness x 1.7
     Per-room tile size overrides the global groutSpec if set.
  */
  const groutDensity = 1.7;
  const groutWasteMultiplier = 1 + (Number(groutSpec.wastePercent) || 0) / 100;
  const L = Math.max(Number(groutSpec.tileLength) || 1, 1);
  const W = Math.max(Number(groutSpec.tileWidth) || 1, 1);
  const J = Math.max(Number(groutSpec.jointWidth) || 1, 1);
  const T = Math.max(Number(groutSpec.tileThickness) || 1, 1);
  const groutKgPerM2 = ((L + W) / (L * W)) * J * T * groutDensity;

  let totalGroutKg = 0;
  rooms.forEach(room => {
    let roomArea = 0;
    room.floorAreas.forEach(fa => { roomArea += (fa.length||0) * (fa.width||0); });
    if (room.useFourWallCalc) {
      roomArea += 2 * ((room.roomLength||0) + (room.roomWidth||0)) * (room.roomHeight||0) - (room.fourWallDeduct||0);
    } else {
      room.walls.forEach(w => { roomArea += (w.length||0) * (w.height||0) - (w.deduct||0); });
    }
    let rKgPerM2;
    if (room.useModularPattern && room.modularTiles && room.modularTiles.length > 0) {
      ...
      if (totalProportion > 0) {
        rKgPerM2 = room.modularTiles.reduce((sum, t) => {
          const tp = tilePresets.find(p => p.label === t.preset);
          const tL = tp ? tp.l : L, tW = tp ? tp.w : W;
      ...
      const rL = preset ? preset.l : L;
      const rW = preset ? preset.w : W;
  ...
  let cementBoardCost = 0;
  let cementBoardSheets = 0;
  let levellingCompoundCost = 0;
  let levellingCompoundBags = 0;
  let levellingCompoundLabourCost = 0;
  let ufhMatCost = 0;
  let ufhLabourCost = 0;
  let ufhThermostatCount = 0;
  let ufhM2 = 0;
  let levellingClipsCost = 0;
  let levellingClipsPacks = 0;
  let cementBoardAdhesiveKg = 0;
  let cementBoardAdhesiveBags = 0;
  let cementBoardAdhesiveCost = 0;
  let cementBoardLabourCost = 0;
  let antiCrackCost = 0;
  let antiCrackAdhesiveKg = 0;
  let antiCrackAdhesiveBags = 0;
  let antiCrackAdhesiveCost = 0;
  let antiCrackLabourCost = 0;
  let tankingWallCost = 0;
  let tankingFloorCost = 0;
  let tankingLabourCost = 0;
  let sealerCost = 0;
  let sealerLabourCost = 0;
  let trimCost = 0;
  let trimLengths = 0;

  // Track which rooms use cement board / anti-crack to avoid double-counting
  const hasCementBoard = rooms.some(r => r.useCementBoard);
  const hasAntiCrack = rooms.some(r => r.useAntiCrack);

  // Adhesive for cement board & anti-crack: Area × 3mm depth = kg needed
  // Formula: m² × 0.003m × 1500 kg/m³ (standard adhesive density) ≈ Area × 4.5 kg/m²
  // Simplified industry standard: Area (m²) × 3 = kg required
  if (hasCementBoard) {
    cementBoardAdhesiveKg = totalFloorArea * 3;
    cementBoardAdhesiveBags = Math.ceil(cementBoardAdhesiveKg / 20);
    cementBoardAdhesiveCost = cementBoardAdhesiveBags * (Number(rates.adhesivePricePerBag) || 0);
  }
  if (hasAntiCrack) {
    antiCrackAdhesiveKg = totalFloorArea * 3;
    antiCrackAdhesiveBags = Math.ceil(antiCrackAdhesiveKg / 20);
    antiCrackAdhesiveCost = antiCrackAdhesiveBags * (Number(rates.adhesivePricePerBag) || 0);
  }

  rooms.forEach((room) => {
    if (room.useCementBoard) {
      const SHEET_AREA = 1.2 * 0.6; // 0.72 m² per sheet
      const sheetsNeeded = Math.ceil(totalFloorArea / SHEET_AREA);
      cementBoardSheets += sheetsNeeded;
      cementBoardCost += sheetsNeeded * (Number(rates.cementBoardPricePerSheet) || 0);
      cementBoardLabourCost += totalFloorArea * (Number(rates.cementBoardLabourPerM2) || 0);
    }
    if (room.useAntiCrack) {
      antiCrackCost += totalFloorArea * rates.antiCrackPerM2;
      antiCrackLabourCost += totalFloorArea * (Number(rates.antiCrackLabourPerM2) || 0);
    }
    if (room.useLevellingCompound) {
      // kg needed = area (m²) × depth (mm) × 1.5 kg/litre density
      // A standard 25kg bag covers approx 3mm over 5m² → coverage = 5 / (depth/3) m² per bag
      const depthMm = Number(room.levellingDepthMm) || 3;
      const coveragePerBag = (5 * 3) / depthMm; // m² per 25kg bag
      const bags = Math.ceil(totalFloorArea / coveragePerBag);
      levellingCompoundBags += bags;
      levellingCompoundCost += bags * (Number(rates.levellingCompoundPricePerBag) || 0);
      levellingCompoundLabourCost += totalFloorArea * (Number(rates.levellingCompoundLabourPerM2) || 0);
    }
    if (room.useUnderfloorHeating) {
      const fa = room.floorAreas.reduce((s,a) => s+(Number(a.length)||0)*(Number(a.width)||0), 0);
      ufhM2 += fa;
      ufhMatCost += fa * (Number(rates.ufhMatPricePerM2) || 0);
      ufhLabourCost += fa * (Number(rates.ufhLabourPerM2) || 0);
      ufhThermostatCount += 1; // 1 thermostat per room
    }
    if (room.useLevellingClips) {
      const roomFloorArea = room.floorAreas.reduce((s,a) => s+(a.length||0)*(a.width||0), 0);
      const clipsPerM2 = getRoomClipsPerM2(room);
      const qty = clipsPerM2 > 0
        ? Math.ceil(roomFloorArea * clipsPerM2)
        : (Number(room.levellingClipsQty) || 0);
    ...
    if (room.useTankingWalls) {
      const tubs = Math.ceil(totalWallArea / 4);
      tankingWallCost += tubs * rates.tankingPerTub;
      tankingLabourCost += totalWallArea * (Number(rates.tankingLabourPerM2) || 0);
    }
    if (room.useTankingFloor) {
      const tubs = Math.ceil(totalFloorArea / 4);
      tankingFloorCost += tubs * rates.tankingPerTub;
      tankingLabourCost += totalFloorArea * (Number(rates.tankingLabourPerM2) || 0);
    }
    if (room.isNaturalStone) {
      sealerCost += totalTileArea * rates.sealerPerM2;
      sealerLabourCost += totalTileArea * (Number(rates.sealerLabourPerM2) || 0);
    }
  ...
  const vatAmount = rates.vatEnabled ? (totalWithMargin * rates.vatRate) / 100 : 0;
                          ...
                          <div className="space-y-3">
                            ...
                            {activeFloor && (
                              <div className="space-y-2">
                                ...
                                <div className="flex gap-1"><Input type="number" placeholder="e.g. 3.2" value={activeFloor.length || ""} onChange={(e)=>updateFloorArea(i,"length",Number(e.target.value))}/><VoiceInput onResult={v=>updateFloorArea(i,"length",Number(v))}/></div>
                                <label className="text-xs text-gray-600">Width (m)</label>
                                <div className="flex gap-1"><Input type="number" placeholder="e.g. 2.4" value={activeFloor.width || ""} onChange={(e)=>updateFloorArea(i,"width",Number(e.target.value))}/><VoiceInput onResult={v=>updateFloorArea(i,"width",Number(v))}/></div>
                            ...
                            <div className="border-t pt-2">
                                        ...
                                        <div className="flex gap-2">
                                          {[100, 150, 200].map(w => (
                                            <button key={w}
                                              ...
                                              onClick={() => updateRoom(i,"ufhWattsPerM2",w)}>
                                              {w}W
                                            </button>
                                          ))}
                                        </div>
                              ...
                              <div className="mt-2">
                                ...
                                <Input type="number" placeholder="0" value={room.trimLength || ""}
                          ...
                          <div className="space-y-3">
                            ...
                            {room.useFourWallCalc && (
                              <div className="space-y-2">
                                ...
                                <Input type="number" value={room.roomLength || ""}
                                ...
                                <Input type="number" value={room.roomWidth || ""}
                                ...
                                <Input type="number" value={room.roomHeight || ""}
                                ...
                                <Input type="number" placeholder="0" value={room.fourWallDeduct || ""}
                            ...
                            {!room.useFourWallCalc && (
                              <div className="space-y-2">
                                ...
                                {activeWall && (
                                  <div className="space-y-2">
                                    <label className="text-xs text-gray-600">Length (m)</label>
                                    <div className="flex gap-1"><Input type="number" placeholder="e.g. 3.2" value={activeWall.length || ""} onChange={(e)=>updateWall(i,"length",Number(e.target.value))}/><VoiceInput onResult={v=>updateWall(i,"length",Number(v))}/></div>
                                    <label className="text-xs text-gray-600">Height (m)</label>
                                    <div className="flex gap-1"><Input type="number" placeholder="e.g. 2.4" value={activeWall.height || ""} onChange={(e)=>updateWall(i,"height",Number(e.target.value))}/><VoiceInput onResult={v=>updateWall(i,"height",Number(v))}/></div>
                                    <div className="flex gap-2">
                                    ...
                                    <Input type="number" placeholder="0" value={activeWall.deduct || ""} onChange={(e)=>updateWall(i,"deduct",Number(e.target.value))}/>
                ...
                <CardContent className="p-4 space-y-4">
                  ...
                  {rates.labourMode === "m2" && (
                    <>
                      ...
                      <Input type="number" value={rates.floorRate} onChange={(e)=>setRates({...rates,floorRate:Number(e.target.value)})}/>
                      <label className="text-xs">Wall Labour (£ per m²)</label>
                      <Input type="number" value={rates.wallRate} onChange={(e)=>setRates({...rates,wallRate:Number(e.target.value)})}/>
                    </>
                  )}
                  ...
                  {rates.labourMode === "day" && (
                    <>
                      ...
                      <Input type="number" value={rates.dayRate} onChange={(e)=>setRates({...rates,dayRate:Number(e.target.value)})}/>
                      <label className="text-xs">Number of Days</label>
                      <Input type="number" value={rates.daysEstimate} onChange={(e)=>setRates({...rates,daysEstimate:Number(e.target.value)})}/>
                    </>
                  )}
                  ...
                  <Input type="number" value={rates.adhesivePricePerBag} onChange={(e)=>setRates({...rates,adhesivePricePerBag:Number(e.target.value)})}/>
                  <label className="text-xs">Adhesive Coverage (m² per 20kg bag)</label>
                  <Input type="number" step="0.5" value={rates.adhesiveCoveragePerBag} onChange={(e)=>setRates({...rates,adhesiveCoveragePerBag:Number(e.target.value)})}/>

                  <label className="text-xs">Cement Board (£ per sheet — 1.2×0.6m)</label>
                  <Input type="number" value={rates.cementBoardPricePerSheet} onChange={(e)=>setRates({...rates,cementBoardPricePerSheet:Number(e.target.value)})}/>
                  <label className="text-xs">Cement Board Install Labour (£ per m²)</label>
                  <Input type="number" value={rates.cementBoardLabourPerM2} onChange={(e)=>setRates({...rates,cementBoardLabourPerM2:Number(e.target.value)})}/>

                  <label className="text-xs">Anti-Crack Membrane (£ per m²)</label>
                  <Input type="number" value={rates.antiCrackPerM2} onChange={(e)=>setRates({...rates,antiCrackPerM2:Number(e.target.value)})}/>
                  <label className="text-xs">Anti-Crack Install Labour (£ per m²)</label>
                  <Input type="number" value={rates.antiCrackLabourPerM2} onChange={(e)=>setRates({...rates,antiCrackLabourPerM2:Number(e.target.value)})}/>

                  <label className="text-xs">Tanking (£ per tub — covers 4 m²)</label>
                  <Input type="number" value={rates.tankingPerTub} onChange={(e)=>setRates({...rates,tankingPerTub:Number(e.target.value)})}/>
                  <label className="text-xs">Tanking Install Labour (£ per m²)</label>
                  <Input type="number" value={rates.tankingLabourPerM2} onChange={(e)=>setRates({...rates,tankingLabourPerM2:Number(e.target.value)})}/>

                  <label className="text-xs">Natural Stone Sealer (£ per m²)</label>
                  <Input type="number" value={rates.sealerPerM2} onChange={(e)=>setRates({...rates,sealerPerM2:Number(e.target.value)})}/>
                  <label className="text-xs">Sealer Install Labour (£ per m²)</label>
                  <Input type="number" value={rates.sealerLabourPerM2} onChange={(e)=>setRates({...rates,sealerLabourPerM2:Number(e.target.value)})}/>

                  <label className="text-xs">⚡ UFH Mat (£ per m²)</label>
                  <Input type="number" value={rates.ufhMatPricePerM2} onChange={(e)=>setRates({...rates,ufhMatPricePerM2:Number(e.target.value)})}/>
                  <label className="text-xs">⚡ UFH Thermostat (£ each)</label>
                  <Input type="number" value={rates.ufhThermostatPrice} onChange={(e)=>setRates({...rates,ufhThermostatPrice:Number(e.target.value)})}/>
                  <label className="text-xs">⚡ UFH Install Labour (£ per m²)</label>
                  <Input type="number" value={rates.ufhLabourPerM2} onChange={(e)=>setRates({...rates,ufhLabourPerM2:Number(e.target.value)})}/>

                  <label className="text-xs">Levelling Compound (£ per 25kg bag)</label>
                  <Input type="number" value={rates.levellingCompoundPricePerBag} onChange={(e)=>setRates({...rates,levellingCompoundPricePerBag:Number(e.target.value)})}/>
                  <label className="text-xs">Levelling Compound Labour (£ per m²)</label>
                  <Input type="number" value={rates.levellingCompoundLabourPerM2} onChange={(e)=>setRates({...rates,levellingCompoundLabourPerM2:Number(e.target.value)})}/>

                  <label className="text-xs">Levelling Clips (£ per 100)</label>
                  <Input type="number" value={rates.levellingClipsPricePer100} onChange={(e)=>setRates({...rates,levellingClipsPricePer100:Number(e.target.value)})}/>
                  ...
                  <div className="border-t pt-2 space-y-2">
                    ...
                    <Input type="number" step="0.01" value={rates.groutPricePerBag} onChange={(e)=>setRates({...rates,groutPricePerBag:Number(e.target.value)})}/>
                    <label className="text-xs text-gray-600">Joint Width (mm)</label>
                    <Input type="number" value={groutSpec.jointWidth} onChange={(e)=>setGroutSpec({...groutSpec,jointWidth:Number(e.target.value)})}/>
                    <label className="text-xs text-gray-600">Tile Thickness (mm)</label>
                    <Input type="number" value={groutSpec.tileThickness} onChange={(e)=>setGroutSpec({...groutSpec,tileThickness:Number(e.target.value)})}/>
                    <label className="text-xs text-gray-600">Waste % (e.g. 10)</label>
                    <Input type="number" value={groutSpec.wastePercent} onChange={(e)=>setGroutSpec({...groutSpec,wastePercent:Number(e.target.value)})}/>
                  </div>

                  <label className="text-xs">Tile Trim (£ per 2.5m length)</label>
                  <Input type="number" step="0.01" value={rates.trimPricePerLength} onChange={(e)=>setRates({...rates,trimPricePerLength:Number(e.target.value)})}/>
                  ...
                  {rates.vatEnabled && (
                    <>
                      ...
                      <Input type="number" value={rates.vatRate} onChange={(e)=>setRates({...rates,vatRate:Number(e.target.value)})}/>
                    </>
                  )}
